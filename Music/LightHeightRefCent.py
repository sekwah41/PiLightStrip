...
while stream.is_active():
    ...
    if len(datasamp):
        ...
        for pixel in range(int(LightInfo.LED_COUNT / 2.0), int(LightInfo.LED_COUNT / 2.0 + levelcolor / 2.0)):
            strip.setPixelColor(pixel,color)
        for pixel in  range(int(LightInfo.LED_COUNT / 2.0),int(LightInfo.LED_COUNT / 2.0 - levelcolor / 2.0), -1):
            strip.setPixelColor(pixel,color)
        strip.show()
